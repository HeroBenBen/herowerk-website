...
test.beforeAll(async () => {
  ...
  server = http.createServer((request, response) => {
    ...
    const relative = url.pathname === '/' ? 'anfrage.html' : decodeURIComponent(url.pathname.slice(1));
...
test('O4 Reader: gültiger v1-Key übernimmt technische Angaben und löscht den Key', async ({ page }) => {
  ...
  const mapped = await page.evaluate(() => Object.fromEntries(buildHubSpotPayload({}).fields.map((field) => [field.name, field.value])));
...
test('O4 Reader: Angaben ändern verwirft den Handoff und stellt den vollständigen Funnel her', async ({ page }) => {
  await setLead(page, VALID_LEAD);
  await page.goto(`${baseURL}/anfrage.html`, { waitUntil: 'domcontentloaded' });
