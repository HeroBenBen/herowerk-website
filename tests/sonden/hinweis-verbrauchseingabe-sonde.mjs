...
async function lauf(heizungWert, andereWert) {
  ...
  await page.route('**/api/rechner.php*', (r) => r.fulfill({ status: 200, contentType: 'application/json', body: '{}' }));
  ...
  for (let i = 0; i < 40; i++) {
    ...
    let wahl = null;
    if (schritt === 15) break;
    const gewaehlt = await page.evaluate(({ heizungWert, andereWert }) => {
      const s = document.querySelector('.wizard-step.active');
      const opts = [...s.querySelectorAll('.wizard-option')];
      if (!opts.length) return 'keine-optionen';
      const wunsch = opts.find((o) => o.dataset.value === heizungWert) ||
      ...
      return ziel.dataset.value + (wunsch ? ' [gewollt]' : '') + ' selected=' + JSON.stringify(sel);
    }, { heizungWert, andereWert });
    await page.evaluate(() => {
      const f = document.querySelector('.wizard-step.active input[type="text"], .wizard-step.active input[type="tel"], .wizard-step.active input[inputmode="numeric"]');
      if (f && !f.value) { f.value = '30159'; f.dispatchEvent(new Event('input', { bubbles: true })); }
    });
    wahl = gewaehlt;
  ...
  const ergebnis = await page.evaluate(() => ({
    ...
    sichtbar: (() => { const e = document.getElementById('wzVerbrauchHinweis');
      if (!e) return false; const r = e.getBoundingClientRect(); return r.width > 0 && r.height > 0; })(),
  }));
  await page.screenshot({ path: process.argv[3] + '/schritt15_' + (andereWert || heizungWert) + '.png', fullPage: false });
  await browser.close();
  return { besuchte, ...ergebnis };
}

console.log('MIT bestehender Waermepumpe:', JSON.stringify(await lauf('sonst', 'waermepumpe'), null, 1));
